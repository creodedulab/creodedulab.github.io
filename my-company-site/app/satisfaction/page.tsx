import Image from "next/image";
import Link from "next/link";

export default function SatisfactionPage() {
  return (
    <div className="relative min-h-screen overflow-hidden bg-white pb-20 pt-32">
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{
          backgroundImage: "url('/intro-bg.jpg')",
          opacity: 0.3,
        }}
      />
      <div className="absolute inset-0 bg-white/70" />

      <div className="relative mx-auto max-w-6xl px-4">
        <div className="mb-20 text-center">
          <h1 className="mb-8 text-4xl font-bold tracking-tight text-indigo-900">
            만족도조사
          </h1>

          <div className="mx-auto max-w-3xl rounded-2xl bg-indigo-50 p-8 shadow-sm md:p-10">
            <h2 className="mb-6 text-2xl font-bold italic text-indigo-700 md:text-3xl">
              "오늘의 배움은 어떠셨나요?"
            </h2>
            <div className="space-y-2 text-lg leading-relaxed text-slate-700 md:text-xl">
              <p>여러분의 경험은 더 나은 교육 과정을 만드는 데 큰 힘이 됩니다.</p>
              <p>잠시 시간을 내어 설문에 참여해 주시면 진심으로 감사드려요.</p>
              <div className="pt-4 font-medium text-slate-600">
                <p>아래에서 해당하는 버튼을 선택하신 뒤</p>
                <p>여러분의 솔직한 경험과 의견을 들려주세요.</p>
              </div>
            </div>
          </div>
        </div>

        <div className="mx-auto grid max-w-4xl gap-8 md:grid-cols-2">
          <div className="group rounded-3xl border-2 border-indigo-100 bg-gradient-to-br from-indigo-50 to-white p-10 text-center shadow-lg shadow-indigo-100/70 transition-all duration-300 hover:border-indigo-200 hover:shadow-2xl">
            <div className="mb-6 text-6xl transition-transform group-hover:scale-110">
              📝
            </div>
            <h3 className="mb-3 text-2xl font-bold text-slate-900">성인반</h3>
            <p className="mb-8 font-medium text-slate-500">
              청년, 직장인, 교직원 과정
            </p>

            <Link
              href="https://docs.google.com/forms/d/e/1FAIpQLSeMWWTzgVJF9wmVHwqrs3vcGzL-ZM6pTYf9O-L-VpiKE_jqDQ/viewform"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block w-full rounded-xl bg-indigo-600 py-4 font-bold text-white shadow-lg transition-colors hover:bg-indigo-700"
            >
              성인반 설문 참여하기 &rarr;
            </Link>

            <div className="mx-auto mt-6 w-1/2 overflow-hidden rounded-2xl border border-indigo-100 bg-white shadow-sm">
              <Image
                src="/adult_survey.png"
                alt="성인반 설문 안내"
                width={1200}
                height={675}
                className="h-auto w-full"
              />
            </div>
          </div>

          <div className="group rounded-3xl border-2 border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-10 text-center shadow-lg shadow-emerald-100/70 transition-all duration-300 hover:border-emerald-200 hover:shadow-2xl">
            <div className="mb-6 text-6xl transition-transform group-hover:scale-110">
              🎒
            </div>
            <h3 className="mb-3 text-2xl font-bold text-slate-900">청소년반</h3>
            <p className="mb-8 font-medium text-slate-500">
              초등, 중등, 고입진로 캠프 과정
            </p>

            <Link
              href="https://docs.google.com/forms/d/e/1FAIpQLSfHxqkrnhCelu3G6yhh3tBzXlbWAGjwKXr5XG3aQylCkXb92g/viewform"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block w-full rounded-xl border-2 border-indigo-600 bg-white py-4 font-bold text-indigo-600 transition-colors hover:bg-indigo-50"
            >
              청소년반 설문 참여하기 &rarr;
            </Link>

            <div className="mx-auto mt-6 w-1/2 overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-sm">
              <Image
                src="/child_survey.png"
                alt="청소년반 설문 안내"
                width={1200}
                height={675}
                className="h-auto w-full"
              />
            </div>
          </div>
        </div>

        <div className="mt-20 text-center text-sm text-slate-400">
          <p>
            여러분의 소감과 피드백은 크레오교육연구소의 다음 교육을 더 단단하게
            만드는 중요한 자료가 됩니다.
          </p>
        </div>
      </div>
    </div>
  );
}
